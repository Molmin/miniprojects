...
class TrainingScoreboardHandler extends Handler {
    ...
    async get(domainId: string, tid: ObjectId) {
        ...
        const rows = []; let datas = [];
        enrollUsers.forEach((userId, uindex) => {
            datas.push(ProblemModel.getListStatus(domainId, userId, pids));
            rows.push([
                { type: 'rank', value: '0' },
                { type: 'user', raw: userId },
                { type: 'string', value: 0 },
            ]);
            for (let i = 0; i < pids.length; i++) {
                rows[uindex].push({
                    type: 'record', score: 0, value: '', raw: undefined,
                });
            }
        });
        datas = await Promise.all(datas);
        enrollUsers.forEach((_uid, uindex) => {
            for (const pid in datas[uindex]) {
            ...
            rows[uindex].forEach((record) => {
                if (record.type === 'record') { rows[uindex][2].value += typeof record.score === 'number' ? record.score : 0; }
            });
        });
